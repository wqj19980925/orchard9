...
import { getToken,getproId,getTaskId,getBaseId,getAdmName } from '@/utils/auth'
import { validateNumber } from "@/utils/validate"
import { myselfConsume } from '@/api/personal'
import { benchStack,benchAddStack, } from '@/api/workbench'
export default {
    data(){
        return{
            title:'任务详情',
    ...
    methods: {
            ...
            benchStack(data).then(success => {
                ...
                if(suc.code == 200){
                    ...
                    for(let i in suc.data){
                        if(num == suc.data[i].task_id){
                            this.myStackDetailInfo = suc.data[i];
                            break;
                        }
                    }
        ...
        smallTasksBtn(val,num){
            ...
            // console.log('单个任务',this.smallTasksListInfo);
            ...
            // this.formData.label_name = val.label;
            // this.formData.form_name = val.fomwork;
        ...
        onSubmitBtn(){
            this.$dialog.confirm({
                ...
                benchAddStack(data).then(success => {
                    this.$toast.clear();//关闭加载层
                    const suc = success.data;
                    if(suc.code == 200){
                        ...
                        var this_ = this;
                        setTimeout(function(){
                            this_.checkShow = false;
                            this_.myStackInfoBtn();
                        },1000);
                    }else{
                        this.$toast.fail(suc.data);
                        return false;
