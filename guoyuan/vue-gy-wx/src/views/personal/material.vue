<template>
    <div>
        ...
        <dt class="mainPersonalListTabBox"><span class="nameBox">物资名称</span><span>库存</span></dt>
        <van-pull-refresh v-if="myMaterialInfo != ''" v-model="isLoading" @refresh="onRefresh" class="mainPersonalBox">
            <ul>
                <li class="listInfoBox" v-for="item in myMaterialInfo" :key="item.index">
                    <span class="nameBox">{{ item.agr_name }}</span><span>{{ item.fma_num }}{{ agrUnit[item.agr_unit-1].state}}</span>
                </li>
            </ul>
        </van-pull-refresh>
        ...
        <van-popup v-model="getShow" position="right" class="mainCheckPopupBox">
            ...
            <van-popup v-model="getLogisticsShow" position="right" class="mainCheckPopupBox">
                <van-nav-bar title="选择仓库和库位" left-text="返回" right-text="完成" left-arrow @click-left="getLogisticsShow=false" @click-right="onSubmitGetBtn"/>
                <div class="mainGetWareBox">
                    <van-cell title="选择仓库" :value="getSubmitFormInfo.rep_name" is-link required @click="storehouseBtn" />
                    <van-cell title="选择库位" :value="getSubmitFormInfo.sto_name" is-link required @click="storehouseSeatBtn"/>
                </div>
...
import { mySelfMaterials,mySelfCheck,mySelfRecord,mySelfRecall,mySelfGetWare,mySelfReturn,mySelfGetStorage } from '@/api/personal'
export default {
    data(){
        return{
            title:'我的物资',
            ...
            getStorehouseSeatInfo:[],//库位列表
            getLogisticsInfo:[],//物流列表
            showPicker:false,
            columns:[],
            getStorLogis:0,
            storehouseIndex:0,
            storehouseSeatIndex:0,
            logisticsIndex:0,
    ...
    methods: {
        ...
        //归还记录 详情
        returnDetailBtn(val){
            for(let i in this.recordInfo.data){
                var id = this.recordInfo.data[i].rec_id
                if(id == val){
                    this.recDetailInfo = this.recordInfo.data[i].agric;
                    break;
                }
            }
        ...
        WarehouseSeatBtn(id){
            const data = {};
            data.rep_id = id;
            mySelfGetStorage(data).then(success => {
                const suc = success.data;
                if(suc.code == 200){
                    this.getStorehouseSeatInfo = suc.data;
                    // console.log('库位',suc.data);
                }else{
                    this.$toast.fail(suc.data);
                    return false;
                }
        ...
        storehouseSeatBtn(){
            var arr = []
            for (let i in this.getStorehouseSeatInfo) {
                arr.push(this.getStorehouseSeatInfo[i].sto_name);
            }
            arr.defaultIndex = this.storehouseSeatIndex
            // console.log(arr);
            this.columns = arr,
            this.getStorLogis = 2;
            this.showPicker = true;
        ...
        onConfirm(val,index) {
            if(this.getStorLogis == 1){
                ...
                this.getSubmitFormInfo.sto_name = '';
                this.WarehouseSeatBtn(this.getSubmitFormInfo.rep_id);
            }else if(this.getStorLogis == 2){
                //库位
                this.storehouseSeatIndex = index;
                this.getSubmitFormInfo.sto_id = this.getStorehouseSeatInfo[index].sto_id;
                this.getSubmitFormInfo.sto_name = this.getStorehouseSeatInfo[index].sto_name;
            }
        ...
        //归还物资 提交
        onSubmitGetBtn(){
            if(this.getSubmitFormInfo.rep_id == '' || this.getSubmitFormInfo.rep_name == '' || this.getSubmitFormInfo.sto_id == ''){
                this.$toast.fail("请选择仓库和库位");
                return false;
            }
