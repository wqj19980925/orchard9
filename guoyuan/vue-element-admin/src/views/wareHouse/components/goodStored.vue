	...
	<div class="app-orderList">
		<el-row class="sd-module-box">
			<div class="sd-moudle-header mar-bot-10">
				<div class="sd-module-title">{{ware_ids.ware_name}} —— 库位</div>
			...
			<div class="lk-page-box" style="padding-top: 10px;">
				<el-pagination :current-page="current_page" :page-size="current_number" :total="totals" layout="total, prev, pager, next, jumper" background @current-change="handleCurrentChange" />
			</div>
			...
			<el-dialog :visible.sync="inventorysGoods" :close-on-click-modal="false" title="盘库" width="700px" @close="addCancel()">
				<div>
				...
				<el-table height="300" :data="goods" width="100%">
					...
					<el-table-column label="盘点数量" width="130" align="center" prop="cond_num" class="detail_num">
						<template slot-scope="{ row }">
			...
			<el-dialog :visible.sync="dishLossMain" :close-on-click-modal="false" title="损耗报备" width="700px" @close="addCancel()">
				<div>
					库位名称：{{ receipt.sto_name }}
					<span style="float: right;">数量：{{ receipt.sto_number }}</span>
				</div>
				<el-table height="300" :data="goods" width="100%" style="margin-bottom: 20px;">
					...
					<el-table-column label="报损数量" width="130" align="center" prop="cond_num" class="detail_num">
						<template slot-scope="{ row }">
			...
			<el-dialog :visible.sync="exportsMains" :close-on-click-modal="false" title="导入库存文件" width="500px" @close="addCancel()">
				<div style="text-align: center;">
					<el-form :model="diaForm" ref="diaForm" label-width="80px">
...
export default {
	...
	methods: {
		getListInformation() {
			...
			that.managesList.ware_id = that.ware_ids.ware_id;
			that.library.rep_id = that.ware_ids.ware_id;
			...
			posiList(ListData)
				.then(response => {
					...
					if (response.data.code == 200) {
						...
						that.totals = response.data.data_info.total;
		...
		resetForm() {
			(this.managesList.sto_name = ''), (this.managesList.sto_state = ''), this.getListInformation();
		},
		addReceipt() {
			this.$emit('submit');
		},
			...
			upFileExcel(el){
			   ...
			   let formData = new FormData();
			    ...
			    formData.append('file',file);
				 formData.append('rep_id',this.ware_ids.ware_id);
				   console.log(formData);
			       lead(formData).then(response=>{
					   console.log(response);
					   const dataRep = response.data
					if(errorStatus(dataRep)) {
					   if(response.data.code=='200'){
