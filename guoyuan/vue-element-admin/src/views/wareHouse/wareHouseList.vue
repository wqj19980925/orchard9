<template>
	<div class="app-orderList">
		<el-row class="sd-module-box" v-if="closeAdd">
			<div class="sd-moudle-header mar-bot-10">
				...
				<div class="sd-module-btn">
					<el-button v-if="jurisdictionList.waremodify=='1'" class="btn-mainCol" @click="centerDialogVisible = true">新增</el-button>
					<el-button v-if="jurisdictionList.warenable=='1'" class="openCol-btn" @click="enable(1)">启用</el-button>
			...
			<el-form class="sd-form-box clearfix" size="medium" label-width="100px" ref="formList" :model="formList">
				<el-row>
					<el-col :span="5" class="sd-col-6 lk-col-5">
						<el-form-item label="冷库名称:" prop="ware_name" class="sd-form-item">
							<el-input v-model="formList.ware_name" placeholder="请输入冷库名称" /></el-form-item>
					</el-col>
					<el-col :span="4" class="sd-col-6" style="width:230px">
						<el-form-item label="库存数量:" prop="warehouse_min" class="sd-form-item">
							<el-input :min="0" type="number" style="width:95%" placeholder="最小值" v-model="formList.warehouse_min" controls-position="right" />
						</el-form-item>
					</el-col>
					<el-col :span="3" class="sd-col-6" style="width:180px">
						<el-form-item label-width="20px" prop="warehouse_max" class="sd-form-item">
							<el-input :min="1" type="number" style="width:75%" placeholder="最大值" v-model="formList.warehouse_max" controls-position="right" />
						</el-form-item>
					</el-col>
					<el-col :span="12" class="sd-col-6" style="width:600px">
						<el-form-item label="冷库地址:" prpo="province" class="sd-form-item inventory">
							<el-select style="width:80px" v-model="formList.province" value-key="id" @change="choseProvince" clearable placeholder="省">
								<el-option v-for="item in province" :key="item.id" :label="item.value" :value="item.value" />
							</el-select>
							<el-select style="width:80px" v-model="formList.city" value-key="id" @change="choseCity" clearable placeholder="市">
								<el-option v-for="item in cityList" :key="item.id" :label="item.value" :value="item.value" />
							</el-select>
							<el-select clearable v-model="formList.county" value-key="id" @change="choseBlock" style="width:80px" placeholder="县/区">
			...
			<div class="lk-table-box amountQuan">
				<el-table :data="tableList" width="100%" @selection-change="handleSelectionChange" @sort-change="sortChang">
					...
					<el-table-column label="库存数量" width="105" align="center" prop="good_number" sortable="custom" class="detail_num" />
					...
					<el-table-column label="操作" align="center">
						<template slot-scope="scope">
							<el-button v-if="jurisdictionList.modifyedit=='1'" class="btn-mainCol" size="mini" @click="compile(scope)">编辑</el-button>
							<el-button v-if="jurisdictionList.writeList=='1'" class="btn-secCol" size="mini" @click="addReceipt03(scope.row)">入库</el-button>
							<el-button v-if="jurisdictionList.index=='1'" class="stopCol-btn" size="mini" @click="addReceipt04(scope.row)">出库</el-button>
							<el-button v-if="jurisdictionList.posiList=='1'" class="openCol-btn" size="mini" @click="addReceipt(scope.row)">库位</el-button>
							<el-button v-if="jurisdictionList.Inventorylist=='1'" class="btn-delete" size="mini" @click="addReceipt02(scope.row)">查库存</el-button>
						</template>
					</el-table-column>
				</el-table>
			</div>
			<div class="lk-page-box" style="padding-top: 10px;">
				<el-pagination :current-page="page" :page-size="page_size" :total="page_count" layout="total, prev, pager, next, jumper" background @current-change="handleCurrentChange" />
			</div>
		</el-row>
		<!-- 添加仓库 -->
		<el-dialog :visible.sync="centerDialogVisible" :close-on-click-modal="false" title="添加冷库" width="500px" @close="addCancel('addCancel')">
			<el-form label-width="85px" ref="addWarehouses" :model="addWarehouses" :rules="rulesCla">
				...
				<el-form-item label="冷库类型:" prop="state" clearable class="sd-form-item">
					<el-select v-model="addWarehouses.state" @change="setIsOk()" style="width:380px" placeholder="请选择冷库类型">
						<el-option v-for="item in type" :key="item.id" :label="item.name" :value="item.id" />
					</el-select>
				</el-form-item>
				<el-form-item label="订单设置:" prop="is_ok" clearable class="sd-form-item">
					<el-select v-model="addWarehouses.is_ok" style="width:380px" :disabled="addWarehouses.state == 1" placeholder="请选择设置状态">
						<el-option v-for="item in is_ok" :key="item.id" :label="item.name" :value="item.id" />
					</el-select>
				</el-form-item>
				<el-form-item label="冷库地址:" prop="province" clearable class="sd-form-item inventory">
					<el-select style="width:122px" v-model="addWarehouses.province" value-key="id" @change="choseProvince" clearable placeholder="省">
						<el-option v-for="item in province" :key="item.id" :label="item.value" :value="item.value" />
					</el-select>
					<el-select style="width:122px" v-model="addWarehouses.city" value-key="id" @change="choseCity" clearable placeholder="市">
						<el-option v-for="item in cityList" :key="item.id" :label="item.value" :value="item.value" />
					</el-select>
					<el-select clearable v-model="addWarehouses.county" value-key="id" @change="choseBlock" style="width:122px" placeholder="县/区">
						<el-option v-for="item in countyList" :key="item.id" :label="item.value" :value="item.value" />
					</el-select>
				</el-form-item>
		...
		<!-- 添加仓库 -->
		<!-- 编辑仓库 -->
		<el-dialog :visible.sync="romewDialogVisible" :close-on-click-modal="false" title="编辑冷库" width="500px" @close="addCancel('editsCancel')">
			<el-form label-width="85px" ref="addWarehouses" :model="addWarehouses" :rules="rulesCla">
				<el-form-item label="冷库名称:" prop="ware_name" class="sd-form-item">
					<el-input v-model="addWarehouses.ware_name" placeholder="请输入冷库名称" style="width:380px" />
				</el-form-item>
		...
		<!-- 添加仓库 -->
		<!--<<库位-->
		<goodStored v-if="showAdd" v-bind:ware_ids="waresid" @submit="submit" @listerToChild="showListPages()" />
		<!--库位>>-->
		<!--<<查库存-->
		<manageExit v-if="showAdd02" v-bind:ware_ids="waresid" @submit="submit" @listerToChild="showListPages()" />
		<!--查库存>>-->
		<!--<<入库-->
		<addReceipt v-if="showAdd03" v-bind:ware_ids="waresid" v-bind:typeId="typeIds" @submit="submit" @listerToChild="showListPages()" />
		<!--入库>>-->
		<!--<<出库-->
		<addReceiptOut v-if="showAdd04" v-bind:ware_ids="waresid" v-bind:typeId="typeIds" @submit="submit" @listerToChild="showListPages()" />
		<!--出库>>-->
	</div>
</template>
<script>
	import { wareList, enableWare, warestop, waredel, addWarehouse, modifyWarehouse } from '@/api/wareHouse';
	...
	export default {
		...
		methods: {
						...
						if(errorStatus(dataRep)) {
							if(response.data.code == 200) {
								...
								for(let i = 0; i < response.data.button.length; i++) {
									if(response.data.button[i].pri_funname == "新增") {
										this.jurisdictionList.waremodify = 1;
									} else if(response.data.button[i].pri_funname == "启用") {
										this.jurisdictionList.warenable = 1;
									} else if(response.data.button[i].pri_funname == "停用") {
										this.jurisdictionList.warestop = 1;
									} else if(response.data.button[i].pri_funname == "删除") {
										this.jurisdictionList.waredel = 1;
									} else if(response.data.button[i].pri_funname == "编辑") {
										this.jurisdictionList.modifyedit = 1;
									} else if(response.data.button[i].pri_funname == "查库存") {
										this.jurisdictionList.Inventorylist = 1;
									} else if(response.data.button[i].pri_funname == "库位") {
										this.jurisdictionList.posiList = 1;
									} else if(response.data.button[i].pri_funname == "入库") {
										this.jurisdictionList.writeList = 1;
									} else if(response.data.button[i].pri_funname == "出库") {
			...
			setIsOk() {
				if(this.addWarehouses.state == 1){
					this.addWarehouses.is_ok = 2
				}else{
					this.addWarehouses.is_ok = ''
				}
			}
		}
	};
