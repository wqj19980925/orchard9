  ...
  <el-row class="sd-module-box lk-addGoods-box">
    ...
    >
      ...
      <el-row :gutter="20" class="addGods-infor-box">
        ...
        <el-col :span="10" class="sd-col-6">
          <el-form-item label="商品编号:" prop="goo_number">
            <el-input
        ...
        <el-col :span="10" class="sd-col-6">
          <el-form-item label="销售价格:" prop="goo_price">
            <el-input v-model="formList.goo_price" placeholder="请输入销售价格" @blur="godsPrice($event)" />
          </el-form-item>
        </el-col>
        <el-col :span="10" class="sd-col-6">
          <el-form-item label="应用导航" prop="nav_id">
            <el-select v-model="formList.nav_id" placeholder="请选择导航名称" style="width:70%;">
              <el-option
            ...
            <el-button type="primary" @click="showNavBoxFun()">增加</el-button>
          </el-form-item>
        </el-col>
        ...
        <el-col :span="24" class="sd-col-6">
          <el-form-item v-if="upImgShow" label="图片列表" prop="goo_picurl" class="addGods-upload-img">
            <div class="upload-img">
              <div v-for="(item,index) in imgArr1" :key="index" class="img-list">
    ...
    <el-dialog
      ...
      append-to-body
    >
      <addnav v-if="navBoxSta" @setInfos='getInfos($event)' ref="navId"></addnav>
      <span slot="footer">
        <el-button @click="cancleForm()">取 消</el-button>
        <el-button type="primary" @click="navSunmit()">提 交</el-button>
      </span>
    </el-dialog>
  </el-row>
</template>
...
export default {
  ...
  methods: {
      ...
      akeyCodeing(list)
        .then(response => {
          ...
          if (errorStatus(dataRep)) {
            ...
            const godsDetail = this.godsDetail;
            let obj = dataRep;
            obj.classId = this.formList.goo_classifi;
            this.$emit("backDataInfo", obj);
            for (const i in godsDetail) {
    ...
    // 触发上传图片事件
    fileClick() {
      document.getElementById("fileIpts").click();
      this.upImgSta = 1;
    },
    // 上传商品图片
    changeGoodsImg(e) {
      const _this = this;
      const imgArrList = _this.imgArr1;
    ...
    // 导航提交
    navSunmit() {
      this.$refs.navId.submit()
    },
    getInfos(val) {
      this.navBoxSta = false;
      this.pageInfor();
      this.formList.nav_id = val;
    }
  }
};
</script>
