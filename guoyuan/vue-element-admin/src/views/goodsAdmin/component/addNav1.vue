  ...
  <div class="app-container">
    <el-row class="sd-module-box">
      ...
      <div class="lk-form-box">
        <el-form ref="navList":model="navList":rules="navRules"label-width="100px"class="lk-form-addGods clearfix">
          <el-form-item label="导航名称"class='lk-el-item'prop="nav_name">
            <el-input class ='lk-input'v-model="navList.nav_name"placeholder="请输入导航名称">
            </el-input>
          </el-form-item>
          <el-form-item label="终端样式" class='lk-el-item'prop="nav_id">
            <el-select v-model="navList.nav_style" >
              <el-option v-for="item in navStyle":key="item.navid":label="item.navname" :value="item.navid"/>
            </el-select>
            <el-button class="text-btn blue"type='text'@click='lokPic()'>
              查看效果
            </el-button>
          </el-form-item>
        </el-form>
      ...
      <div class="">
        <el-container>
          ...
          <el-aside width="450px"class='addNav-left-box'>
            <div class='navTip-box'>
              ...
              <div class="navTip-title-box">
                <div class='navTitle'@click="navSort">
                  {{navTip.title}}
                </div>
              </div>
              <!-- 第一屏 不开启脱拽 -->
              <ul class='nav-no-sort'v-show='navTip.sortShow'>
                <li class='itemMainBox'id='itemMainBox'v-for='(item,index) in navTipList':key='index'
                  @mouseover="item.showTool=true"@mouseout="item.showTool=false">
                  <div :class='item.chose?"itemsBox on":"itemsBox"'>
                    <div class="imgsBox"@click="handTip(item.icon_order)">
                      <img :src="item.img"class='img_nav'>
                    </div>
                    <p class="names"v-show='!item.showTool'>
                      {{item.name}}
                    </p>
                    <p class="btnItem"v-show='item.showTool'>
                      <span class="looks tool_tip"@click="item.showEye=!item.showEye"type="1"title="是否显示该导航按钮">
                        <i class="el-icon-view"v-if='item.showEye'></i>
                        <svg-icon icon-class="hidden"v-else />
                      </span>
                      <label class="replaces tool_tip"title="替换该导航按钮图片"data="1"@click="uploadFile(item.icon_order)">
                        <i class="el-icon-refresh"></i>
                      </label>
                      <span class="deletes tool_tip"type="1"title="删除该导航按钮"@click='delTip(item.icon_order,item.system_icon)'>
                ...
                <li class="upload-img itemMainBox"v-if='navTipList.length<10'>
                  <div class='itemsBox'>
                    <div class="imgsBox uploadimg">
                      <p class="upload"@click='uploadImg(navTipList.length)'>
                        +
                      </p>
                    </div>
              ...
              <input type="file"style='display:none;'id="fileIpt"accept="image/*"@change="changeImg($event)">
              <!-- 第二屏-拖拽 -->
          ...
          <el-main class='addNav-right-box'>
            <el-form :model="addNavForm":rules="addNavForm"label-width="80px"ref='addNavForm'class='lk-el-form'>
              <el-form-item label='信息名称'>
                <el-input placeholder="请输入信息名称"v-model='addNavForm.name'style='width:360px':maxlength="6"
                @blur="inforName($event)"/>
                <span class='tip_box'>提示:为确保移动端效果,此项最多输入6个字符</span>
              </el-form-item>
              <el-form-item label='信息类型'>
                <el-input v-show='!navType.showSel'placeholder=""v-model='addNavForm.type2'style='width:360px'
                :disabled="navType.selDisabled"/>
                <el-select style='width:360px'v-show='navType.showSel'v-model='addNavForm.type2'@change='navTypeChose($event)'
                  :disabled="navType.selDisabled">
                  <el-option v-for='type1 in typeNav':key='type1.id':label="type1.label":value="type1.id">
                  </el-option>
                </el-select>
              </el-form-item>
              <div class='right_bottom_detail'v-show='navType.show'>
                ...
                <div class='text-box'v-show="navType.navType===1">
                  ...
                  <div class=''>
                    <el-row class="addGods-infor-box":gutter="10">
                      <div v-for='(nav,ind) in navDetailArr':key='ind'>
                        <div v-for="(item,index) in nav":key="index"class="addGods-infor-ipt clearfix">
                          <el-row v-if='item.icon_order==navChanged.choseId':gutter="5">
                            <el-col :span="9">
                              <el-input v-model="item.key":class="item.nothing1?'border-red':''"placeholder="长度限制8位"
                                :maxlength="8"@blur="textTitlBlur($event.index)"/>
                            </el-col>
                            <el-col :span="13"class="add-gods-title">
                              <el-input v-model="item.value":class="item.nothing2?'border-red':''"placeholder=""/>
                            </el-col>
                            <el-col :span="2"class="add-gods-title">
                              <el-button :disabled="nav.length===1"icon="el-icon-delete"class="btn-delete"
                  ...
                  <el-row>
                    <el-button type='primary'class='add-btn'@click='addBtn'>
                      新增
                    </el-button>
                  </el-row>
                </div>
                ...
                <div class="url-box"v-show="navType.navType===3">
                  <div v-for='(url,index) in urlArr':key='url.icon_order'>
                    <div v-if='url.icon_order==navChanged.choseId'>
                      <el-input v-model="url.imgText"placeholder="请输入网址"style='width:360px;'@blur='urlCheck($event,index)'/>
                    </div>
                  </div>
                </div>
              </div>
...
export default {
  ...
  data () {
    return {
      ...
      navRules: {
        nav_name: [{ min: 1, max: 20, message: '导航名称过长,字数不可超过20!', trigger: 'blur' }],
        nav_style: [{ required: true, trigger: 'change', message: '请选择样式导航' }],
      },
      navTip: {
        title: '开启手动拖拽排序',
        sortShow: true,
  ...
  methods: {
    getPageInfo () {
      ...
      if (edit_id === 0) {
        ...
        addNav(data).then(response => {
          ...
          if (errorStatus(response.data)) {
            ...
            for (let i in dataRep) {
              ...
              this.$set(this.navTipList[i], 'icon_order', dataRep[i].icon_order);//系统自带导航的 唯一标识 1产品信息 2溯源真伪 3防伪查询 
              ...
              this.$set(this.navTipList[i], 'system_icon', dataRep[i].system_icon); //是否是系统固有的导航
            ...
            this.$set(this.navTipList[2], 'type_cont', '防伪管理中的真伪设置信息');
          }
        }).catch(Error => {
          loading.close();
        ...
        data.nav_id = edit_id;
        _this.tit = '编辑';
        editNav(data).then(response => {
          loading.close();
          if (errorStatus(response.data)) {
            this.navList.nav_name = response.data.data.nav_name;
            const dataRep = response.data.data.nav_icon;
            console.log("编辑--", dataRep)
            for (let i in dataRep) {
              const obj1 = {};
              _this.navTipList.push(obj1);
              _this.$set(_this.navTipList[i], 'name', dataRep[i].name); //导航名称
              _this.$set(_this.navTipList[i], 'icon_order', dataRep[i].icon_order);//系统自带 唯一标识 1产品信息 2溯源真伪 3防伪查询
              _this.$set(_this.navTipList[i], 'icon_type', dataRep[i].icon_type); //
              _this.$set(_this.navTipList[i], 'img', str + dataRep[i].pic);
              _this.$set(_this.navTipList[i], 'showEye', dataRep[i].show);
              _this.$set(_this.navTipList[i], 'system_icon', dataRep[i].system_icon);
              this.$set(this.navTipList[i], 'content', { key: [], value: [] }); // 右侧内容
              _this.$set(_this.navTipList[i], 'chose', false);
              this.$set(this.navTipList[i], 'showTool', false); //判断是否显示导航下部操作按钮---自加
              if (dataRep[i].icon_order === "1") { //产品信息
                _this.$set(_this.navTipList[i], 'type_cont', '商品管理中的商品信息');
              } else if (dataRep[i].icon_order === "2") { //溯源真伪
                _this.$set(_this.navTipList[i], 'type_cont', '溯源管理中的溯源信息');
              } else if (dataRep[i].icon_order === "3") { //防伪查询
                _this.$set(_this.navTipList[i], 'type_cont', '防伪管理中的真伪设置信息');
              }
              // 自定义导航 content ---内容--循环设置
              if (dataRep[i].system_icon === "false") {
                this.navDetailArr.push([{
                  key: '',
                  value: '',
                }])
                this.textArr.push({
                ...
                const len1 = Number(this.navDetailArr.length) - 1;
                console.log("len1", len1)
                var navTextDet = []
                if (dataRep[i].icon_type === 1) {
                  for (var j = 0; j < dataRep[i].content.key.length; j++) {
                    console.log("dataRep[i].content.key", dataRep[i].content.key)
                    if (j < 1) {
                      this.$set(this.navDetailArr[len1][j], 'key', dataRep[i].content.key[j]);
                    } else {
                      this.navDetailArr[len1].splice(j, 0, {})
                      this.$set(this.navDetailArr[len1][j], 'key', dataRep[i].content.key[j])
                    }
                    for (var k = 0; k < dataRep[i].content.value.length; k++) {
                      console.log("dataRep[i].content.value", dataRep[i].content.value)
                      if (k === j) {
                        console.log("k-", k)
                        this.$set(this.navDetailArr[len1][k], 'value', dataRep[i].content.value[k]);
                        break;
                      }
                    }
                    this.$set(this.navDetailArr[len1][j], 'icon_order', dataRep[i].icon_order)
                  }
                } else if (dataRep[i].icon_type === 2) {
                  this.$set(this.textArr[len1], 'imgText', dataRep[i].content)
                  this.$set(this.textArr[len1], 'icon_order', dataRep[i].icon_order)
                } else {
                  this.$set(this.urlArr[len1], 'imgText', dataRep[i].content)
                  this.$set(this.urlArr[len1], 'icon_order', dataRep[i].icon_order)
                }
              }

            }
            console.log("this.navTipList", _this.navTipList)
          }
        }).catch(Error => {
          loading.close();
    ...
    submit () {  
      ...
      if (this.navList.nav_style === "全部" || this.navList.nav_style === 0 ) {
        this.navList.nav_style =''
      ...
      var navName = [], navTYpe = [], navCont = [], navIcon = [], navPic = [], nacShow = [], system_icon = [], icon_order = [];
            ...
            if (navdetail[j][0].icon_order === this.navTipList[i].icon_order) {
              ...
              for (let k in navdetail[j]) {
                if (navdetail[j][k].key === '') {
                  ...
                  this.$message.error("自定义导航信息内容存在空项，请您添加完成后再点击保存！");
                  return false
                  break;
                }
      ...
      data.nav_style = this.navList.nav_style;
      data.name = navName;
        ...
        for (let i in navTipList) {
          ...
          if (id === navTipList[i].icon_order) {
            ...
            if (navTipList[i].system_icon == "true") {
              ...
              this.addNavForm.type2 = this.typeNav[Number(navTipList[i].icon_type) - 1].label;// 下拉框显示 文本    
              this.$set(this.navType, 'selDisabled', false); // 去掉下拉框的不可选择
              this.$set(this.navType, 'showSel', true); //
              this.$set(this.navChanged, 'navDetaiInd', navTipList[i].icon_order - 5);
    ...
    uploadFile (id) {
      ...
      this.navChanged.changeImg=true;
    },
    // 上传图片--建立新的导航
    uploadImg (id) {
       this.navChanged.changeImg=false;
      var icon_order = null;
      if (id === 3) {
        icon_order = 5;
        this.navDetailArr = [];
    ...
    changeImg (e) {
      const _this = this;
      const navTipList = _this.navTipList;
      const imgId = _this.navChanged.ImgId;
      ...
      reader.onload = function (e) {
        ...
        if (!_this.navChanged.changeImg) {
          ...
          const nav = { 'name': '新导航', showTool: false, showEye: true, chose: true, content: '', icon_order: imgId, icon_type: 1, system_icon: 'false', content: { 'key': [], 'value': [] } };
          nav.img = imgcode;
          _this.$set(_this.addNavForm, 'name', '新导航');
          _this.$set(_this.navType, 'show', true); // 右侧显示文本输入框
          ...
          const leng1 = _this.navDetailArr.length;
    ...
    navTypeChose (e) {
      ...
      this.$confirm('切换后已写入内容会清空,请确认是否切换?', '提示', {
        ...
        const index = this.navChanged.navDetaiInd;
        if (e != 1) {
        ...
        for (var i = 0; i < navTipList.length; i++) {
          ...
          if (navTipList[i].icon_order == this.navChanged.choseId) {
            ...
            if (e == 1) {
              this.$set(this.navDetailArr[index][0], 'icon_order', navTipList[i].icon_order);
              break
            } else if (e == 2) {
              // console.log("e-2-", e)
              this.$set(this.textArr[index], 'icon_order', navTipList[i].icon_order)
              break
            } else if (e == 3) {
              // console.log("e-3-", e)
              this.$set(this.urlArr[index], 'icon_order', navTipList[i].icon_order)
              break
            }
          }
        }
