                      ...
                      <div v-for='(nav,ind) in navDetailArr':key='ind'>
                        <div v-for="(item,index) in nav":key="index"class="addGods-infor-ipt clearfix">
                          <el-row v-if='item.icon_order==navChanged.choseId':gutter="5">
                            ...
                            <el-col :span="13"class="add-gods-title">
                              <el-input maxlength="30" v-model="item.value":class="item.nothing2?'border-red':''"placeholder=""/>
                            </el-col>
                            <el-col :span="2"class="add-gods-title">
  ...
  methods: {
      ...
      if (edit_id === 0) {
        ...
        editNav(data).then(response => {
          ...
          if (errorStatus(response.data)) {
            ...
            this.navList.nav_style = response.data.data.nav_style;
            const dataRep = response.data.data.nav_icon;
            ...
            for (let i in dataRep) {
              ...
              } else if (dataRep[i].icon_order === 4) { //环境监测
                _this.$set(_this.navTipList[i], 'type_cont', '产区管理中的环境信息');
              } else if (dataRep[i].icon_order === 5) { //质检报告
                _this.$set(_this.navTipList[i], 'type_cont', '溯源管理中的发码信息');
              }
        ...
        for (let i in navTipList) {
          ...
          if (id === navTipList[i].icon_order) {
            ...
            if (navTipList[i].system_icon == true) {
              ...
              this.$set(this.navChanged, 'navDetaiInd', navTipList[i].icon_order - 6);
    ...
    changeImg (e) {
      ...
      reader.onload = function (e) {
        ...
        if (!_this.navChanged.changeImg) {//是否是更改图片
          _this.navChanged.choseId = imgId;
          // 建 新导航
          for (let i in navTipList) {
            navTipList[i].chose = false;//把当前选中的导航取消选中
          }
          const nav = { 'name': '新导航', showTool: false, showEye: 1, chose: true, content: '', icon_order: imgId, icon_type: 1, system_icon: 'false', content: { 'key': [], 'value': [] } };
          nav.img = imgcode;
