    ...
    <div class="sd-moudle-header mar-bot-10">
     ...
     <el-form ref="formList":model="formList":rules="formRules"label-width="150px"class="lk-form-addGods">
      <el-row :gutter="20" class="addGods-infor-box">
        ...
        <el-col :span="12" class="sd-col-6">
          <el-form-item label="已发码量:" class="sd-form-item" prop="pro_code">
            {{formList.pro_code}}
...
export default {
  ...
  this.searchQuality()
  if(this.senThiscode ===true){
    this.disabl=true
    this.formList.far_id = this.far_name
    this.formList.base_id = this.base_name
    this.formList.pro_id = this.pro_number
    this.changeBase(this.base_id)
    this.changeNum(this.pro_id)
  ...
  methods: {
      ...
      searchSendCode(search)
        .then(response => {
          ...
          if (errorStatus(dataRep)) {
            ...
            if(this.senThiscode ===true){
              this.formList.pro_id = this.pro_number
              this.formList.cla_name = this.cla_name
              this.formList.goo_name = this.goo_name
              this.formList.pro_estimate = this.pro_estimate
              this.formList.pro_code = this.pro_code
            }
    ...
    changeNum(e){
      ...
      for(var i in this.NumList){
        if(e === this.NumList[i].pro_id){
          ...
          goo_id = this.NumList[i].goo_id
        }
      }
      this.searchQuality(2,goo_id)
      const data = {};
      data.type=2
    ...
    submitInfo(formName) {
        ...
        if(this.formList.code_num === '' || Number(this.formList.code_num) === 0){
        ...
        // var baserurl="http://gy.bluearp.com"
        const loading = this.$loading({
          lock: true,
          text: 'Loading',
          spinner: 'el-icon-loading',
        ...
        .then(response => {
          ...
          if (errorStatus(dataRep)) {
            ...
            this.$router.push('../Code/CodeManageindex');
          }
        })
        .catch(Error => {
          loading.close()
