  ...
  <div class="app-container">
    ...
    >
      <div class="sd-moudle-header mar-bot-10">
        <div class="sd-module-title">模板列表</div>
      ...
      <el-form ref="formList":model="formList":rules="formRules"class="sd-form-box clearfix"size="medium"label-width="100px">
        ...
        <el-col :span="6"class="sd-col-6">
          <el-form-item label="标签名称"class="sd-form-item"prop="fla_name">
        ...
        <el-col :span="6"class="sd-col-6">
          <el-form-item label="状态"prop="fom_state"> 
            <el-select v-model="formList.fom_state"placeholder class="form-ipt-sed">
    ...
    <div class="lk-table-box">
      <el-table :data="list"stripe style="width: 100%"highlight-current-row @selection-change="handleSelectionChange">
        ...
        <el-table-column label="时间"min-width prop="fom_cretime"align="center"/>
        <el-table-column label="状态"min-width align="center">
          <template slot-scope="scope">
            <div v-if="scope.row.fom_state === 1">启用</div>
            <div v-else>停用</div>
          </template>
        </el-table-column>
        <el-table-column label="操作"width="320"prop align="center">
          <template slot-scope="scope">
            <el-button v-if="btnShow.editBtn" class="btn-mainCol"size="mini"@click="editBtn(scope.row.fom_id)">
              编辑
            </el-button>
            <el-button v-if="btnShow.copyBtn" class="btn-secCol"size="mini"@click="copyData(scope.row.fom_id)">
              复制
            </el-button>
            <el-button v-if="scope.row.fom_state==2 && btnShow.startBtn"size="mini"class="openCol-btn"@click="enableThisClas(scope.row,1)">
              启用
            </el-button>
            <el-button v-if="scope.row.fom_state==1 && btnShow.stopBtn"size="mini"class="stopCol-btn"@click="enableThisClas(scope.row,2)">
              停用
            </el-button>
    ...
    <addtrace v-if="!showList":edit-id="edit_id"@listerToChild="showListPages()":edit='editShow'/>
  </div>
</template>

<script>
import { TempList, tempStart, tempStop, AddTemp, editTemp, delTemp, copyTemp, searTemp } from '@/api/trace'
...
export default {
  ...
  created() {
    // 页面加载时
    this.getPageInfo()
    this.formList.fom_state = this.state[0].name
  },
  methods: {
    getPageInfo() {
      const data = {}
    ...
    enableClas() {
      ...
      if (chosed.length < 1) {
        this.$message.error('请选择要启用的模板!')
      } else {
        const id = []
        const data = {}
        for (const i in chosed) {
          if (chosed[i].fom_state === 2) {
            id.push(chosed[i].fom_id)
          }
        }
        // console.log('选择的数据-id-', id)
        if (id.length > 0) {
          data.fom_id = id
          this.$confirm('请确认,是否启用选中的模板?', '提示', {
    ...
    stopClas() {
      ...
      const chosed = this.chosed
      if (chosed.length < 1) {
        this.$message.error('请选择要停用的模板!')
      } else {
        const id = []
        const data = {}
        for (const i in chosed) {
          if (chosed[i].fom_state === 1) {
            id.push(chosed[i].fom_id)
          }
        }
        // console.log('选择的数据-id-', id)
    ...
    delChoseBtn() {
      ...
      if (chosed.length < 1) {
        ...
        for (const i in chosed) {
          data.fom_id.push(chosed[i].fom_id)
        }
        this.$confirm('请确认,是否删除所选中的模板?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
